...
### D-015 — The moderator UI authenticates individual moderators through Discord OAuth2 and authorises them from Discord permissions and role IDs; the shared DASHBOARD_TOKEN stays a machine credential only

**Context**: dashboard.py has exactly one auth tier: a single shared DASHBOARD_TOKEN compared with secrets.compare_digest, no sessions and no CSRF exposure because every route is a read. Adding writes turns it into a real web application. database.resolve_pending and database.resolve_appeal both record resolved_by, expecting a Discord user id — a shared bearer token cannot supply one, so every ban approved from the browser would carry a falsified or empty audit trail. Separately, SECURITY.md records that delegated scope authorship was deferred to exactly this gameplan, and INVARIANT-05 requires permission and role-ID checks rather than name strings.
**Decision**: Moderator identity for the UI comes from Discord OAuth2: the moderator signs in with Discord, and the resulting user id is the identity recorded in resolved_by. Authorisation reuses the same permission and role-ID facts the slash commands use, resolved per guild, so a moderator can do in the browser exactly what they can do in Discord and no more. DASHBOARD_TOKEN is retained only for machine-to-machine reads (health checks, scrapes) and is never accepted on a state-mutating route. Sessions are cookie-based with SameSite=Strict and a CSRF token on every mutating form.
**Consequences**: The UI acquires an OAuth client registration, a redirect URI, a client secret in the environment, and a session store — all new deployed surface, and the loopback-only posture must become a TLS-terminating reverse proxy before any moderator can reach it. This is the largest new attack surface in the gameplan and it is why the UI is last. Delegated scope authorship (which roles may edit which scopes) becomes designable here rather than deferred again.
**Evidence**: dashboard.py create_app and require_token; database.py resolve_pending and resolve_appeal record resolved_by; docs/SECURITY.md section 2 defers delegation to the moderator UI; INVARIANT-05; docs/DEPLOYMENT.md lines 90-94 warn never to move DASHBOARD_HOST off loopback.
**Status**: active (2026-09-07)
